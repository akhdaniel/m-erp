import jwt
import json

# The new JWT token
token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..cp049Ggyd9wTHfD0b4kWDumMnG1Aznt34qL3uIIcfu4"

# Try to decode without verification first to see the payload
try:
    payload = jwt.decode(token, options={"verify_signature": False})
    print("Payload without verification:")
    print(json.dumps(payload, indent=2))
    
    # Check if email is in the payload
    if 'email' in payload:
        print(f"\nEmail found in payload: {payload['email']}")
    else:
        print("\nEmail NOT found in payload")
        
    # Check if user_id is in the payload
    if 'user_id' in payload:
        print(f"User ID found in payload: {payload['user_id']}")
    else:
        print("User ID NOT found in payload")
        
    # Check if permissions are in the payload
    if 'permissions' in payload:
        print(f"Permissions found in payload: {len(payload['permissions'])} permissions")
    else:
        print("Permissions NOT found in payload")
        
except Exception as e:
    print(f"Error decoding without verification: {e}")

# Try to decode with verification using the secret key
secret_key = "development-secret-key-change-in-production"
try:
    payload = jwt.decode(token, secret_key, algorithms=["HS256"])
    print("\nPayload with verification:")
    print(json.dumps(payload, indent=2))
except Exception as e:
    print(f"Error decoding with verification: {e}")